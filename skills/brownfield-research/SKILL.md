...
> "No Brownfield config found. Running general research without stack-specific context. Run `/brownfield:brownfield-init` to configure your project for better results."
...
[How this option interacts with their configured stack. If no config, write "Configure with /brownfield:brownfield-init for stack-specific guidance."]
```
...
2. **Convert to workstream** — Pick an option and I'll save this research so `/brownfield:brownfield-plan` can skip its research phase and jump straight to planning.
...
Generated by /brownfield:brownfield-research on [date]
...
> "Research saved to `.brownfield/workstreams/<id>/research-preload.md`. When you run `/brownfield:brownfield-plan`, it will detect this file and skip its own research phase, using your pre-validated findings instead."
...
> "No expert domains configured. Research is based on general industry patterns. Run `/brownfield:brownfield-init` to add expert domains for deeper, more targeted research."
