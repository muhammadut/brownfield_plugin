...
Read `.brownfield/config.json` from the project root.
...
If `.brownfield/config.json` does not exist or is incomplete, proceed with basic research but inform the user:

> "No Brownfield config found. Running general research without stack-specific context. Run `/brownfield:init` to configure your project for better results."
...
### Agent A: `brownfield:researcher` — Industry Research
...
### Agent B: `brownfield:expert-researcher` — Expert Domain Research
...
[How this option interacts with their configured stack. If no config, write "Configure with /brownfield:init for stack-specific guidance."]
```
...
2. **Convert to workstream** — Pick an option and I'll save this research so `/brownfield:plan` can skip its research phase and jump straight to planning.
...
1. **Generate a workstream ID using the SAME format `plan` uses**: take the first 3-4 significant words from the user's research question, slugify them (lowercase, hyphens, no special chars), then append today's date as `YYYYMMDD`.

   Example: question `"Should we use Redis or Memcached for session caching?"` → workstream id `redis-vs-memcached-session-20260413`.

   **Important:** do NOT use `ws-<slug>-<unix-timestamp>` format. The ID must match exactly what `/brownfield:plan` would generate for the same feature description on the same day, otherwise the preload handoff will not be found by plan's Phase 3.0 check. Ask the user to confirm the slug if it's ambiguous.

2. **Create the research preload file**:
   - Path: `.brownfield/workstreams/<id>/research-preload.md`
   - Contents:

```markdown
# Research Preload: [Topic]
Generated by /brownfield:research on [date]
...
> "Research saved to `.brownfield/workstreams/<id>/research-preload.md`. When you run `/brownfield:plan`, it will detect this file and skip its own research phase, using your pre-validated findings instead."
...
> "No expert domains configured. Research is based on general industry patterns. Run `/brownfield:init` to add expert domains for deeper, more targeted research."
