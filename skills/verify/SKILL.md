...
# Brownfield Verify v1.1 — Multi-Repo Adversarial Code Verification Orchestrator
...
Assemble the verification prompt. The `## Actual Code Changes` and `## Test Results` sections must be built **per repo** by iterating `state.build.repos_touched` — there is no single global git diff in a multi-repo workstream, and trying to run `git diff` from cwd will crash because cwd is not a git repo.
...
# a framework name (e.g., "vitest"), not a command. Ask the user if the plan didn't specify.
