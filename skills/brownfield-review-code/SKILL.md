...
# Brownfield Review Code — Dual Reviewer System

You are the Brownfield code review orchestrator. Your job is to launch **two independent reviewers** with different lenses, run them in parallel, and synthesize their findings into a single attributed report.
...
# For --pr flag (use GitHub CLI):
gh pr view <id> --json title,body,files,headRefName,baseRefName
gh pr diff <id>
...
**For `--workstream <id>`:** Also read `.brownfield/workstreams/<id>/plan.md`. Extract the "Feature Request & Clarifications" section verbatim. This becomes the "Stated Intent" passed to both reviewers. If plan.md doesn't exist, warn the user and proceed without intent.

**For `--pr <id>` without `gh` CLI:** Detect by running `gh --version` first. If `gh` is missing or unauthenticated, stop and tell the user:
> "GitHub CLI (`gh`) is not installed or not authenticated. Either install/authenticate it (`brew install gh && gh auth login`), or use `--branch <name>` or `--commits <range>` instead. For non-GitHub hosts (GitLab, Bitbucket, Azure DevOps), use the branch/commits flags."
...
```
Agent(
  subagent_type="brownfield:pattern-detector",
  ...
  prompt="
    ...
    ## Output Path
    .brownfield/reviews/<review-id>/pattern-context.md
...
Create directory: `.brownfield/reviews/<review-id>/`
...
Agent(
  ...
  prompt="
    ...
    ## Output Format
    Write your review to .brownfield/reviews/<review-id>/architect-review.md
...
- If `"codex"`: write adversary prompt to `.brownfield/reviews/<review-id>/adversary-prompt.md`, then run `codex exec --full-auto`
...
codex exec "$(cat .brownfield/reviews/<review-id>/adversary-prompt.md)" --full-auto \
  -o .brownfield/reviews/<review-id>/adversary-review.md
...
- `.brownfield/reviews/<review-id>/architect-review.md`
- `.brownfield/reviews/<review-id>/adversary-review.md`

Synthesize into `.brownfield/reviews/<review-id>/combined-review.md`:
...
Full review: .brownfield/reviews/oauth-login-20260405-1430/combined-review.md
```
...
- Save all review artifacts to `.brownfield/reviews/<review-id>/` for audit trail
...
| --pr flag but no `gh` CLI | Fall back to asking user for the branch/commits |
| Codex not available | Both reviewers are Claude with different prompts (Architect/Adversary) |
| No config.json (Brownfield not initialized) | Still works — skip pattern-detector step, run reviewers with diff-only |
