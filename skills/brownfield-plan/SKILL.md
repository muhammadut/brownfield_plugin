...
Check `.brownfield/workstreams/` for existing workstreams:

Use Glob to scan: `.brownfield/workstreams/*/state.json`
...
```bash
mkdir -p .brownfield/workstreams/<id>/agent-outputs
```
...
```
<cwd>/.brownfield/workstreams/<id>/agent-outputs/
```

Store this as `agent_outputs_dir_abs` — you'll pass it to every spawned agent.

Write `.brownfield/workstreams/<id>/state.json`:
```json
{
  "id": "<workstream-id>",
...
Update `.brownfield/workstreams/<id>/state.json`:
...
```
Agent(
  subagent_type="brownfield:researcher",
...
```
Agent(
  subagent_type="brownfield:expert-researcher",
...
Read `.brownfield/learning/codebase-patterns.md` and `.brownfield/learning/lessons-learned.md` if they exist. These contain lessons from past workstreams that should inform this plan.
...
```
Agent(
  subagent_type="brownfield:code-explorer",
...
```
Agent(
  subagent_type="brownfield:security-analyzer",
...
Read every file in `.brownfield/workstreams/<id>/agent-outputs/` directly. These are the full-fidelity outputs from researcher, expert-researcher, pattern-detector, code-explorer, and security-analyzer. Use them verbatim for synthesis — do NOT rely on summaries.

### 5.2 Write the Plan Document

Write `.brownfield/workstreams/<id>/plan.md` with this EXACT structure:
...
- Lessons applied: <list of lessons from .brownfield/learning/ that were relevant, or "None — first workstream">
...
<Any relevant entries from .brownfield/learning/*.md.
...
- Apply relevant lessons from .brownfield/learning/
...
### Issues (for each):
  ...
  - Suggestion: <fix>" \
  --full-auto \
  -o .brownfield/workstreams/<id>/review-raw.md
```
...
4. Write review decisions to `.brownfield/workstreams/<id>/review-decisions.md`
