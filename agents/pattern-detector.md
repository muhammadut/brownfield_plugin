...
5. **Prior Agent Outputs** -- paths to researcher and expert-researcher output files in `.brownfield/workstreams/<id>/agent-outputs/`. Read these BEFORE starting your analysis. They contain industry best practices and domain patterns that you must compare existing repo code against.
...
7. **Output Path** -- the absolute path where you must write your findings (e.g., `.brownfield/workstreams/<id>/agent-outputs/03-pattern-detector.md`).
...
Then read:
- `.brownfield/workstreams/<id>/agent-outputs/01-researcher.md` — industry patterns, methodologies, pitfalls
- `.brownfield/workstreams/<id>/agent-outputs/02-expert-researcher-*.md` — domain-specific guidance
